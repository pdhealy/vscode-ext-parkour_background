...
function updateMenuContext(theme: string): void {
    vscode.commands.executeCommand('setContext', 'backgroundImage.enabled', theme === 'minecraft' || theme === 'subwaysurfers');
}
...
async function applyThemeForWindow(context: vscode.ExtensionContext, theme: string, silent = false) {
    if (theme === 'minecraft' || theme === 'subwaysurfers') {
        ...
        const opacity = config.get<number>('opacity', 0.10);
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('backgroundImage.on', async () => {
        const picked = await vscode.window.showQuickPick(['Minecraft', 'Subway Surfers'], {
            placeHolder: 'Select a theme to enable Parkour Background',
        });
        if (!picked) { return; }
        ...
        const themeValue = picked === 'Minecraft' ? 'minecraft' : 'subwaysurfers';
        await config.update('activeTheme', themeValue, target);
    }));

    context.subscriptions.push(vscode.commands.registerCommand('backgroundImage.off', async () => {
        const config = vscode.workspace.getConfiguration('backgroundImage');
        _localConfigChange = true;
        const target = vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0 
            ? vscode.ConfigurationTarget.Workspace 
            : vscode.ConfigurationTarget.Global;
        await config.update('activeTheme', 'none', target);
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('backgroundImage.setOpacity', async () => {
        const PRESETS = [
            ...
            { label: '10% — Default',     value: 0.10 },
            { label: '15% — Medium', value: 0.15 },
            { label: '25% — High',    value: 0.25 },
        ];
        const config = vscode.workspace.getConfiguration('backgroundImage');
        const current = config.get<number>('opacity', 0.10);
